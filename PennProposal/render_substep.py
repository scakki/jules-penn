...
# ============================================================
# Substep logging — monkey-patches do_simulation
# ============================================================
def install_substep_logger(env):
    """
    Monkey-patch env.do_simulation to capture state at every 2ms substep.

    After the rollout, call `env._substep_buf` to get the list of dicts,
    or `flush_substep_csv(env, path)` to write to disk.

    Captures per substep:
      - ctrl targets (same across substeps within one control step)
      - qfrc_actuator (DIFFERENT each substep — this is what we need)
      - qpos (joints + base)
      - qvel (joints + base)
    """
    env._substep_buf = []
    ...
    _original_do_simulation = env.do_simulation

    def do_simulation_with_logging(ctrl, n_frames):
        env.data.ctrl[:] = ctrl

        for s in range(n_frames):
            mujoco.mj_step(env.model, env.data)

            # Actual torque the PD produced at THIS substep
            tau = np.array([env.data.qfrc_actuator[d] for d in env._dof_adrs])

            # Joint positions & velocities
            q  = np.array([float(env.data.qpos[env._qadr[jid]])
                           for jid in env._joint_ids])
            qd = np.array([float(env.data.qvel[d]) for d in env._dof_adrs])

            env._substep_buf.append({
                "ctrl_step": env._substep_ctrl_step,
                "substep":   s,
                "time":      float(env.data.time),
                # Base state (including quaternion — critical for replay)
                "base_x":    float(env.data.qpos[0]),
                "base_y":    float(env.data.qpos[1]),
                "base_z":    float(env.data.qpos[2]),
                "base_qw":   float(env.data.qpos[3]),
                "base_qx":   float(env.data.qpos[4]),
                "base_qy":   float(env.data.qpos[5]),
                "base_qz":   float(env.data.qpos[6]),
                # Base velocities
                "base_vx":   float(env.data.qvel[0]),
                "base_vy":   float(env.data.qvel[1]),
                "base_vz":   float(env.data.qvel[2]),
                "base_wx":   float(env.data.qvel[3]),
                "base_wy":   float(env.data.qvel[4]),
                "base_wz":   float(env.data.qvel[5]),
                # Ctrl targets (same for all substeps in one control step)
                **{f"ctrl_{i}": float(ctrl[i]) for i in range(6)},
                # Actual PD torques (DIFFERENT each substep!)
                **{f"tau_{i}":  float(tau[i])  for i in range(6)},
                # Joint positions
                **{f"q_{i}":    float(q[i])    for i in range(6)},
                # Joint velocities
                **{f"qd_{i}":   float(qd[i])   for i in range(6)},
            })

        env._substep_ctrl_step += 1

    # Replace the method
    env.do_simulation = do_simulation_with_logging
    print(f"[substep logger] Installed on env. Logging every substep.")


def flush_substep_csv(env, filepath):
    """Write the substep buffer to CSV."""
    buf = env._substep_buf
    if not buf:
        print("[substep logger] No data to flush.")
        return
    ...
    print(f"[substep logger] Wrote {len(buf)} substep rows → {filepath}")
    print(f"  ({len(buf) // 5} control steps × 5 substeps)")
...
def render_best(run_folder):
    ...
    # ── Install substep logger on the unwrapped env ──
    install_substep_logger(env)
    ...
    # ── Flush substep CSV ──
    substep_csv_path = os.path.join(plots_dir, "substeps.csv")
    flush_substep_csv(env, substep_csv_path)
