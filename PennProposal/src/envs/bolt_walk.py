...
class BoltEnvWalk(MujocoEnv, utils.EzPickle):
    metadata = {
        "render_modes": [
            "human",
            "rgb_array",
    ...
    def step(self, action):
        ...
        sigma = 0.5   # velocity tolerance
        forward_reward = self._forward_reward_weight * np.exp(- ((vx - self.v_Des) ** 2) / (2 * sigma**2)) * heading_gate
        ...
        reward = (
            forward_reward + alive_bonus - ctrl_cost - vz_penalty
        )
